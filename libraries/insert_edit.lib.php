...
function PMA_showEmptyResultMessageOrSetUniqueCondition($rows, $key_id,
    ...
    if (! $rows[$key_id]) {
        ...
        list($unique_condition, $tmp_clause_is_unique) = PMA_Util::getUniqueCondition(
...
function PMA_getMaxUploadSize($column, $biggest_max_file_size)
{
    ...
    $html_output = PMA_Util::getFormattedMaximumUploadSize(
            $this_field_max_size
        ) . "\n";
    ...
    if (is_null($current_row[$column['Field']])) {
        ...
        if ($column['is_binary']
            ...
            if ($_SESSION['tmp_user_values']['display_binary_as_hex']
                ...
                $current_row[$column['Field']] = PMA_Util::replaceBinaryContents(
                        $current_row[$column['Field']]
                    );
            }
        } // end if
...
function PMA_setSessionForEditNext($one_where_clause)
{
    ...
    list($unique_condition, $clause_is_unique) = PMA_Util::getUniqueCondition(
...
function PMA_transformEditedValues($db, $table,
    ...
    foreach ($edited_values as $cell_index => $curr_cell_edited_values) {
        if (isset($curr_cell_edited_values[$column_name])) {
            ...
            $extra_data['transformations'][$cell_index] = $transformation_plugin->applyTransformation(
                    $column_data,
                    $transform_options,
                    ''
                );
...
function PMA_getQueryValuesForInsertAndUpdateInMultipleEdit($multi_edit_columns_name,
    ...
    if ($is_insert) {
        ...
        $query_values[] = PMA_Util::backquote($multi_edit_columns_name[$key])
