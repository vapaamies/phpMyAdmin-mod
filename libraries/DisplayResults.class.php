...
class PMA_DisplayResults
{
    ...
    private function _getTableHeaders(
        ...
        if ($is_display['sort_lnk'] == '1') {
            ...
            list($unsorted_sql_query, $drop_down_html) = $this->_getUnsortedSqlAndSortByKeyDropDown(
        ...
        // text button or link
        list($colspan, $rowspan, $button_html) = $this->_getFieldVisibilityParams(
        ...
        for ($j = 0; $j < $this->__get('fields_cnt'); $j++) {
            ...
            if (($is_display['sort_lnk'] == '1') && ! $is_limited_display) {
                ...
                list($order_link, $sorted_headrer_html) = $this->_getOrderLinkAndSortedHeaderHtml(
    ...
    private function _getFieldVisibilityParams(
        ...
        if ((($is_display['edit_lnk'] == self::NO_EDIT_OR_DELETE)
            ...
            $vertical_display['emptypre'] = (($is_display['edit_lnk'] != self::NO_EDIT_OR_DELETE)
    ...
    private function _getSortOrderLink(
        ...
        if ($GLOBALS['cfg']['HeaderFlipType'] == self::HEADER_FLIP_TYPE_AUTO) {
            ...
            $GLOBALS['cfg']['HeaderFlipType'] = (PMA_USR_BROWSER_AGENT == 'IE')
    ...
    private function _getColumnAtRightSide(
        ...
        if ((($GLOBALS['cfg']['RowActionLinks'] == self::POSITION_RIGHT)
            ...
            $vertical_display['emptyafter'] = (($is_display['edit_lnk'] != self::NO_EDIT_OR_DELETE)
    ...
    private function _getTableBody(
        ...
        $odd_row = true;
        $directionCondition = ($_SESSION['tmp_user_values']['disp_direction'] == self::DISP_DIR_HORIZONTAL)
            || ($_SESSION['tmp_user_values']['disp_direction'] == self::DISP_DIR_HORIZONTAL_FLIPPED);
        ...
        while ($row = PMA_DBI_fetch_row($dt_result)) {
             ...
             */
            list($where_clause, $clause_is_unique, $condition_array) = PMA_Util::getUniqueCondition(
                    $dt_result,
            ...
            $del_url = $del_query = $del_str = $edit_anchor_class = $edit_str = $js_conf = $copy_url = $copy_str = $edit_url = null;
            ...
            if (($is_display['edit_lnk'] != self::NO_EDIT_OR_DELETE)
                ...
                if ($is_display['edit_lnk'] == self::UPDATE_ROW) {
                    ...
                    list($edit_url, $copy_url, $edit_str, $copy_str,
                        $edit_anchor_class) = $this->_getModifiedLinks(
                                $where_clause,
                ...
                if (($is_display['del_lnk'] == self::DELETE_ROW)
                    ...
                    list($del_query, $del_url, $del_str, $js_conf) = $this->_getDeleteAndKillLinks(
    ...
    private function _getRowValues(
        ...
        for ($currentColumn = 0;
            ...
            if ($meta->numeric == 1) {
                ...
                $vertical_display['data'][$row_no][$i] = $this->_getDataCellForNumericColumns(
                ...
                $vertical_display['data'][$row_no][$i] = $this->_getDataCellForBlobColumns(
                ...
                $vertical_display['data'][$row_no][$i] = $this->_getDataCellForGeometryColumns(
                ...
                $vertical_display['data'][$row_no][$i] = $this->_getDataCellForNonNumericAndNonBlobColumns(
                        $row[$i], $class, $meta, $map, $_url_params,
                        $condition_field, $transformation_plugin,
                        $default_function, $transform_options,
    ...
    private function _gatherLinksForLaterOutputs(
        ...
        if (!empty($del_url)
            ...
            $vertical_display['row_delete'][$row_no] .= $this->_getCheckboxForMultiRowSubmissions(
    ...
    private function _getDataCellForGeometryColumns(
        ...
        if (! isset($column) || is_null($column)) {
            ...
            } elseif ($_SESSION['tmp_user_values']['geometry_display'] == self::GEOMETRY_DISP_WKT
            ) {
    ...
    public function setConfigParamsForDisplayTable()
    {
        ...
        $_SESSION['tmp_user_values']['query'][$sql_md5]['sql'] = $this->__get('sql_query');
        ...
        if (PMA_isValid(
            ...
            $_SESSION['tmp_user_values']['query'][$sql_md5]['display_text'] = self::DISPLAY_PARTIAL_TEXT;
        }
        ...
        if (PMA_isValid(
            ...
            $_SESSION['tmp_user_values']['query'][$sql_md5]['relational_display'] = self::RELATIONAL_KEY;
        }
        ...
        if (PMA_isValid(
            ...
            $_SESSION['tmp_user_values']['query'][$sql_md5]['geometry_display'] = self::GEOMETRY_DISP_GEOM;
        }
        ...
        $_SESSION['tmp_user_values']['display_binary'] = isset($_SESSION['tmp_user_values']['query'][$sql_md5]
                ['display_binary']
            )
            ? true
            : false;
        $_SESSION['tmp_user_values']['display_binary_as_hex'] = isset($_SESSION['tmp_user_values']['query'][$sql_md5]
                ['display_binary_as_hex']
            )
            ? true
            : false;
        $_SESSION['tmp_user_values']['display_blob'] = isset($_SESSION['tmp_user_values']['query'][$sql_md5]['display_blob'])
            ? true
            : false;
        $_SESSION['tmp_user_values']['hide_transformation'] = isset($_SESSION['tmp_user_values']['query'][$sql_md5]
                ['hide_transformation']
            )
            ? true
            : false;
