...
class PMA_ServerStatusData
{
    ...
    public function __construct()
    {
        ...
        $links['table'][__('Flush (close) all tables')] = $this->selfUrl . '?flush=TABLES&amp;' . PMA_generate_common_url();
        $links['table'][__('Show open tables')] = 'sql.php?sql_query=' . urlencode('SHOW OPEN TABLES') .
                '&amp;goto=' . $this->selfUrl . '&amp;' . PMA_generate_common_url();

        if ($GLOBALS['server_master_status']) {
            $links['repl'][__('Show slave hosts')] = 'sql.php?sql_query=' . urlencode('SHOW SLAVE HOSTS')
                    . '&amp;goto=' . $this->selfUrl . '&amp;'
        ...
        $links['qcache'][__('Flush query cache')] = $this->selfUrl . '?flush=' . urlencode('QUERY CACHE') . '&amp;' .
                PMA_generate_common_url();
        ...
        $links['innodb'][__('Variables')] = 'server_engines.php?engine=InnoDB&amp;' . PMA_generate_common_url();
        $links['innodb'][__('InnoDB Status')] = 'server_engines.php?engine=InnoDB&amp;page=Status&amp;' .
                PMA_generate_common_url();
