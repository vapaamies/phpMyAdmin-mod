...
if (! PMA_Util::cacheExists('mysql_charsets', true)) {
    ...
    while ($row = PMA_DBI_fetch_assoc($res)) {
        ...
        $mysql_charsets_available[$row['CHARACTER_SET_NAME']] = !empty($mysql_charsets_available[$row['CHARACTER_SET_NAME']])
...
function PMA_generateCharsetDropdownBox($type = PMA_CSDROPDOWN_COLLATION,
    ...
    foreach ($mysql_charsets as $current_charset) {
        ...
        $current_cs_descr = empty($mysql_charsets_descriptions[$current_charset])
            ? $current_charset
