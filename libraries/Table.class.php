...
class PMA_Table
{
    ...
    static public function analyzeStructure($db = null, $table = null)
    {
        ...
        if (self::isView($db, $table)) {
            ...
            foreach ($results as $result) {
                $analyzed_sql[0]['create_table_fields'][$result['COLUMN_NAME']] = array('type' => strtoupper($result['DATA_TYPE']));
            }
        } else {
    ...
    protected function saveUiPrefsToDb()
    {
        ...
        if ($rows_count > $max_rows) {
            ...
            $sql_query = ' DELETE FROM ' . $pma_table .
    ...
    protected function loadUiPrefs()
    {
        ...
        if (! isset($_SESSION['tmp_user_values']['table_uiprefs'][$server_id][$this->db_name][$this->name])) {
            ...
            $_SESSION['tmp_user_values']['table_uiprefs'][$server_id][$this->db_name][$this->name] = (strlen($GLOBALS['cfg']['Server']['pmadb'])
