...
function PMA_getStuffForEngineTypeTable($current_table, $db_is_information_schema,
    ...
    case 'PBMS' :
        ...
        list($current_table, $formatted_size, $unit, $sum_size) = PMA_getValuesForPbmsTable($current_table, $is_show_stats, $sum_size);
        //$display_rows                   =  ' - ';
        break;
...
function getHtmlForRowStatsTable($showtable, $tbl_collation,
    ...
    if (!$is_innodb
        ...
        list($avg_row_length_value, $avg_row_length_unit) = PMA_Util::formatByteDown(
...
function PMA_getHtmlForActionsInTableStructure($type, $tbl_storage_engine,
    ...
    $html_output .= $primary;
    list($unique, $unique_enabled) = PMA_getHtmlForActionRowInStructureTable(
            $type, $tbl_storage_engine,
            'unique nowrap',
    ...
    $html_output .= $unique;
    list($index, $index_enabled) = PMA_getHtmlForActionRowInStructureTable(
            $type, $tbl_storage_engine,
    ...
    if (!PMA_DRIZZLE) {
        ...
        list($spatial, $spatial_enabled) = PMA_getHtmlForActionRowInStructureTable(
                $type, $tbl_storage_engine,
                'spatial nowrap',
...
function PMA_moveColumns($db, $table)
{
    ...
    for ($i = 0, $l = count($_REQUEST['move_columns']); $i < $l; $i++) {
        ...
        $default_type = $data['Null'] === 'YES' && $data['Default'] === null
                ? 'NULL'
                : ($current_timestamp
                    ? 'CURRENT_TIMESTAMP'
                    : ($data['Default'] === null
