...
function PMA_getMessageForUpdatePassword($err_url, $username, $hostname)
{
    ...
    if (empty($message)) {
        ...
        $hashing_function = (! empty($_REQUEST['pw_hash']) && $_REQUEST['pw_hash'] == 'old'
                ? 'OLD_'
                : ''
            )
            . 'PASSWORD';
...
function PMA_getListOfPrivilegesAndComparedPrivileges()
{
    ...
    $list_of_compared_privileges = '`Select_priv` = \'N\''
        . ' AND `Insert_priv` = \'N\''
        . ' AND `Update_priv` = \'N\''
        . ' AND `Delete_priv` = \'N\''
        . ' AND `Create_priv` = \'N\''
...
function PMA_getUsersOverview($result, $db_rights, $link_edit, $pmaThemeImage,
    ...
    $html_output = '<form name="usersForm" id="usersForm" action="server_privileges.php" '
        . 'method="post">' . "\n"
        . PMA_generate_common_hidden_inputs('', '')
